#!/usr/bin/env python3
"""analytics_agent.py - Stats YouTube + cerebro_knowledge | Cron 8h00 UTC"""
...
SYSTEM=("Analista performance canal psicologia.doc. "
        "Padroes sucesso/fracasso baseados em dados. JSON valido sem markdown.")

def sb_get(table, qs, lim=50):
    ...
    for attempt in range(3):
        try:
            ...
            log("sb_get "+table+" ("+str(attempt+1)+"/3): "+str(e)[:60]); time.sleep(3)
    return []
...
def yt_stats(ids):
    ...
    return {item["id"]:{"title":item.get("snippet",{}).get("title",""),
...
def run():
    ...
    published=sb_get("content_pipeline",
        "status=eq.published&published_at=gte."+cutoff
        +"&select=id,title,youtube_id,youtube_long_id,format,viral_score"
        "&order=published_at.desc",50)
    log("Videos 30d: "+str(len(published)))
    if not published: swarm_report({"status":"no_data"}); return

    yt_ids=list({v.get("youtube_id") or v.get("youtube_long_id","") for v in published
                 if v.get("youtube_id") or v.get("youtube_long_id")})
    real=yt_stats(yt_ids)
    log("Stats reais: "+str(len(real)))

    video_data=[{"title":v.get("title","")[:55],"format":v.get("format","?"),
                 "score":v.get("viral_score",0),
                 "views":real.get(v.get("youtube_id") or v.get("youtube_long_id",""),{}).get("views","N/A")}
                for v in published]
    ...
    cerebro=sb_get("cerebro_knowledge","select=titulo,categoria,confidence_score&order=confidence_score.desc",8)
    ...
    prompt=("Analise performance 30d canal psicologia.doc.\n\n"
            ...
            "Extraia 3-5 insights NOVOS. "
            '{"insights":[{"category":"title|format|topic|timing|thumbnail|engagement",'
    ...
    try:
        ...
        data=json.loads(clean_json(raw)); insights=data.get("insights",[]); summary=data.get("weekly_summary","")
    except Exception as e: log("Erro: "+str(e)[:100]); swarm_report({"status":"error"}); return

    log(str(len(insights))+" insights extraidos"); saved=0
    for ins in insights:
        if not ins.get("insight"): continue
        status,_=sb_put("cerebro_knowledge",{
            "categoria":ins.get("category","performance"),
            "subcategoria":ins.get("category","algorithm"),
            "titulo":ins["insight"][:120],
            "conteudo":{"insight":ins["insight"],"evidence":ins.get("evidence",""),"action":ins.get("action",""),"week":WEEK_ID},
            "fonte":"analytics-agent-"+WEEK_ID,
            "confidence_score":int(ins.get("confidence",70)),
            "aprendido_em":TODAY,"ultima_verificacao":TODAY,"ativo":True,
        })
        if status in (200,201): saved+=1; log("  OK ["+ins.get("category","")+"] "+ins["insight"][:55])
    ...
    log("Analytics: "+str(saved)+" insights -> cerebro_knowledge")
