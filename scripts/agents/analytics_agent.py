...
def run():
    ...
    yt_ids = list({v.get("youtube_id") or v.get("youtube_long_id", "")
                   for v in published
                   if v.get("youtube_id") or v.get("youtube_long_id")})
    real = yt_stats(yt_ids)
    log("Stats reais: " + str(len(real)))

    video_data = [{"title":  v.get("title", "")[:55],
                   "format": v.get("format", "?"),
                   "score":  v.get("viral_score", 0),
                   "views":  real.get(v.get("youtube_id") or v.get("youtube_long_id", ""),
                                      {}).get("views", "N/A")}
                  for v in published]
    wv = sorted([x for x in video_data if isinstance(x["views"], int)],
                key=lambda x: x["views"], reverse=True)
    top_ctx = ("\n".join("+" + str(v["views"]) + "v | " + v["title"] + " | " + v["format"]
                          for v in wv[:5]) or "Sem dados reais")
    bot_ctx = ("\n".join("-" + str(v["views"]) + "v | " + v["title"] + " | " + v["format"]
                          for v in wv[-5:]) if len(wv) >= 5 else "")

    cerebro = sb_get("cerebro_knowledge",
                     "select=titulo,categoria,confidence_score&order=confidence_score.desc", 8)
    cerebro_ctx = ("\n".join("[" + c.get("categoria", "") + "| " +
                              str(c.get("confidence_score", 0)) + "] " +
                              c.get("titulo", "")[:80] for c in cerebro)
                   if cerebro else "Sem dados")
    ...
    for ins in insights:
        ...
        if status in (200, 201):
            saved += 1
            log("  OK [" + ins.get("category", "") + "] " + ins["insight"][:55])

    memory_store("analytics:" + WEEK_ID, json.dumps({"summary": summary, "insights": saved}))
    report({"status": "done", "week_id": WEEK_ID, "insights_saved": saved, "videos": len(published)})
