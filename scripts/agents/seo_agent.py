...
def run():
    ...
    decisions=sb_get("strategy_decisions","week_id=eq."+WEEK_ID+"&select=id,selected_topics",1)
    if not decisions:
        log("Sem strategy — aguardando"); swarm_report({"status":"waiting_strategy"}); return

    decision=decisions[0]; strategy_id=decision["id"]
    ...
    opps_all=sb_get("research_opportunities","week_id=eq."+WEEK_ID+"&select=*",20)
    saved=0
    for t in selected:
        topic=t.get("topic","")
        ...
        status,_=sb_put("seo_metadata",{
            "week_id":WEEK_ID,"topic":topic[:200],
        ...
        if status in (200,201):
            ...
            if best:
                sb_put("content_pipeline",{
                    ...
                    "status":"pending_generation","format":t.get("format","short"),
