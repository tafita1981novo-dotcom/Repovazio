...
def render(vid):
    ...
    for i,para in enumerate(paras):
        ...
        r2=ffrun(["-y","-loop","1","-t",str(spc+0.5),"-i",img,
                  "-vf",f"scale={W}:{H},fps=30,zoompan=z='min(zoom+{zs},{zm})':d={d}:s={W}x{H}:fps=30",
                  "-c:v","libx264","-crf","18","-preset","fast",
                  "-b:v","5000k","-r","30","-pix_fmt","yuv420p","-t",str(spc),clip],120)
        if r2.returncode==0: clips.append(clip)
        else: log(f"   ⚠️ KB: {r2.stderr.decode()[-60:]}")
