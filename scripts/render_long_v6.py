...
def imagem_pollinations(prompt, neg, out, seed):
    ...
    for a in range(3):
        try:
            ...
            if len(data)>10000:  # Minimo 10KB para imagem valida
                open(out,"wb").write(data)
                log(f"    Img Pollinations: {len(data)//1024}KB OK")
                return True
            else:
                log(f"    Img Pollinations: {len(data)}B muito pequena, tentando novamente")
        except Exception as e:
            log(f"    Img tent {a+1}: {e}")
        time.sleep(8*(a+1))
    return False

def imagem_proc(out, seed, tema):
    """Fallback procedural garantido"""
    try:
        ...
        log(f"    Img procedural OK")
        return True
    except Exception as e:
        err(f"Proc img: {e}"); return False

def get_imagem(prompt, neg, out, seed, tema):
    """Tenta Pollinations, fallback procedural"""
    if imagem_pollinations(prompt,neg,out,seed):
        # Validar tamanho
        try:
            if pathlib.Path(out).stat().st_size > 10000: return True
        except: pass
