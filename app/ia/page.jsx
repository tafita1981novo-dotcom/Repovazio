...
export default function Chat(){
  ...
  async function send(){
    ...
    const fileData=file?.data;setFile(null);if(arenaOnRef.current&&text){runArena(text,newMsgs);return;}
  ...
  return(
    ...
    <div className="app">
      ...
      <main className="main">
        ...
        {/* INPUT */}
        <div className="ia">
          {voiceOn&&(<div onClick={function(e){if(e.currentTarget===e.target)vToggle();}} style={{position:'fixed',inset:0,zIndex:9999,background:'rgba(6,6,15,0.96)',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)'}}><div style={{textAlign:'center',maxWidth:'90%',padding:24}}><div style={{fontSize:84,marginBottom:18,filter:voiceListening?'drop-shadow(0 0 28px #7C3AED)':'none',transition:'all .25s'}}>{voiceBusy?'💭':voiceListening?'🎙️':'🎤'}</div><div style={{color:'#fff',fontSize:21,marginBottom:10,fontWeight:600}}>{voiceBusy?'Pensando...':voiceListening?'Ouvindo... pode falar':'Toque em Falar e fale'}</div><div style={{color:'#A78BFA',fontSize:16,minHeight:26,marginBottom:26,fontStyle:'italic'}}>{voiceText}</div><div style={{display:'flex',gap:12,justifyContent:'center'}}><button onClick={vRec} disabled={voiceBusy||voiceListening} style={{padding:'14px 30px',borderRadius:30,border:'none',background:'#7C3AED',color:'#fff',fontSize:17,fontWeight:700,opacity:(voiceBusy||voiceListening)?0.45:1}}>🎤 Falar</button><button onClick={vToggle} style={{padding:'14px 30px',borderRadius:30,border:'1px solid #555',background:'transparent',color:'#fff',fontSize:17}}>✕ Encerrar</button></div><div style={{marginTop:18,display:'flex',gap:8,flexWrap:'wrap',justifyContent:'center'}}>{['Aoede','Kore','Leda','Zephyr'].map(function(vn){return (<button key={vn} onClick={()=>{voiceNameRef.current=vn;setVoiceName(vn);}} style={{padding:'7px 14px',borderRadius:18,border:'1px solid #444',background:voiceName===vn?'#5B21B6':'transparent',color:'#fff',fontSize:13}}>{vn}</button>);})}</div><div style={{color:'#777',fontSize:12,marginTop:16}}>Voz neural Gemini · {voiceName} · PT-BR</div></div></div>)}{file&&(<div className="fp">{file.preview?<img src={file.preview} className="fp-img" alt="preview"/>:<span>📎 {file.name}</span>}<button onClick={()=>setFile(null)}>✕</button></div>)}
          <div className="ic">
            <button className="ub2" onClick={()=>fileRef.current?.click()} title="Anexar">📎</button>
            <input ref={fileRef} type="file" accept="*/*" onChange={handleFile} style={{display:'none'}}/><button className="ub2" onClick={vToggle} title="Conversar por voz">🎤</button><button className="ub2" onClick={()=>{const nv=!arenaOnRef.current;arenaOnRef.current=nv;setArenaOn(nv);}} title="Arena: varios modelos competem" style={{background:arenaOn?'#7C3AED':undefined,color:arenaOn?'#fff':undefined}}>⚔️</button>
          ...
          <div className="in">Daniela V21 🎤📄🔊⚔️ · Groq{useQwen?' + Qwen3':''} · Giphy · Browser · Skills · MCP · 24/7</div>
        </div>
      </main>
